...
for (i in 1:1000) {
    ...
    s<- summary(l)
    sbeta.oh[i] <- s$coefficients[1,1]
    beta.x[i] <- s$coefficients[2,1]
    
}
beta.oh
beta.x
betas<-cbind(beta.oh, beta.x)
...
Standard error of Bo and Bx:

```{r}
betas<- as.data.frame(betas)
betas$ss.b0 <- (beta.oh-mean(betas$beta.oh))^2  #sum of squares b0
betas$ss.bx <- (beta.x-mean(betas$beta.x))^2 #sum of squares bx
...
95% confidence intervals for Bo and Bx
```{r}
#95%CI
#bo
confint(betas$beta.oh, pnorm, level=0.95)
lower.bo<- quantile(betas$beta.oh, probs=0.025)
upper.bo<- quantile(betas$beta.oh, probs=0.975)
lower.bo
upper.bo
```
So there is a 95% chance that the true value of the intercept falls between -10.7 and -8.4. 
```{r}
#bo
confint(betas$beta.oh, pnorm, level=0.95)
...
SD.comp
```
The standard errors are  smaller in the bootstrapped data. 
...
CI.all
```
The confidence intervals are fractionally smaller with the bootstrapped data, but practically the same. Not sure how to explain this. 
...
v This was my stab at this but it didn't go far.
```{r}
